...
Hydra-style brute-forcer for Jenkins form login (Rust binary: `jenkins_brute`). Sends POST requests to the configured login endpoint (default: `j_spring_security_check`) and treats a successful login based on the HTTP `Location` header (redirect handling disabled).

**Use only on systems you own or are explicitly authorized to test** (e.g. bug bounty scope, lab, CTF).
...
**Linux example** (adjust the tag if needed):

```bash
curl -fsSL -o jenkins_brute \
  "https://github.com/abdulsaabir/Jenkins-login-brute-forcer/releases/download/v0.1.0/jenkins_brute-linux-x86_64"
